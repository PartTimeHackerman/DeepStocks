...
@Service
public class StockTradingTimesDBUpdater implements DataBaseUpdater<TradingTimes> {
	
	private final StockTradingTimesDAO stockTradingTimesDAO;
	private final StockRepo stockRepo;
	
	@Autowired
	public StockTradingTimesDBUpdater(StockTradingTimesDAO stockTradingTimesDAO, StockRepo stockRepo) {
		this.stockTradingTimesDAO = stockTradingTimesDAO;
		this.stockRepo = stockRepo;
	}
	
	@Override
	public void update(TradingTimes tradingTimes) {
		List<StockTradingTimes> stockTradingTimes = getStockTradingTimes(tradingTimes);
		stockTradingTimesDAO.save(stockTradingTimes);
	}
	
	public void update(Collection<TradingTimes> tradingTimesCollection) {
		tradingTimesCollection.forEach(this::update);
	}
	
	
	public List<StockTradingTimes> getStockTradingTimes(TradingTimes tradingTimes) {
